...
try
{
	...
	{			
		...
		$requete="SELECT *,a.id as id_printer,b.id as id_marque,b.detail as marque,c.detail as modele,a.id_type as id_type_printer FROM printer a, marque b, modele c WHERE a.id_modele=c.id AND c.id_marque=b.id";// 
...
else if($_GET["action"] == "create")
	{
		...
		$prep = NULL;
		$requete = "INSERT INTO printer(id_materiel,id_modele,name,driver,port,sn";
		if (!empty($_POST["date_installe"])){
			$requete.=",date_installe";
		}
		$requete.=",createOn,updateOn) VALUES(:id_materiel,:id_modele,:name,:driver,:port,:sn";
		if (!empty($_POST["date_installe"])){			
			$requete.=",:date_installe";
		}
		$requete.=",now(),now())";		
		...
		$prep->bindParam(":id_modele",$resMarque["id"],PDO::PARAM_INT);
                $prep->bindParam(":name",$_POST["name"],PDO::PARAM_STR);		
		...
		$requete = "SELECT * FROM printer WHERE id = :id";
		$prep=$db->prepare($requete);
	...
	{		
		...
		$requete = "UPDATE printer SET id_modele=:id_modele,name=:name,driver=:driver,port=:port,sn=:sn";
		if (!empty($_POST["date_installe"])){
			$tmpDate=explode('-',$_POST["date_installe"]);
			$_POST["date_installe"]="$tmpDate[2]-$tmpDate[1]-$tmpDate[0]";
		...
		$prep=$db->prepare($requete);
		$prep->bindParam(":id",$_POST["id_printer"],PDO::PARAM_INT);
	...
	else if($_GET["action"] == "delete")
	{
		...
		$requete="DELETE FROM printer WHERE id = :id;";		
		$prep=$db->prepare($requete);
		$prep->bindParam(":id",$_POST["id_printer"],PDO::PARAM_INT);
		$prep->execute();
		$prep->closeCursor();
