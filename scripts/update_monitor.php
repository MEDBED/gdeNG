...
try
{
	...
	{			
		...
		$requete="SELECT *,a.id as id_monitor,b.id as id_marque,b.detail as marque,c.detail as modele,a.id_type as id_type_monitor FROM monitor a, marque b, modele c WHERE a.id_modele=c.id AND c.id_marque=b.id";// 
		foreach ($tabFiltre as $search=>$champ){
			if (!empty($_POST[$champ])){$requete.=" AND $search LIKE :$champ";}
		}
		$requete.=" AND id_materiel=:id_materiel ORDER BY marque LIMIT ".$_GET["jtStartIndex"]."," . $limHaute . ";";		
...
else if($_GET["action"] == "create")
	{
		...
		$prep = NULL;
		$requete = "INSERT INTO monitor(id_materiel,id_modele,sn";
		if (!empty($_POST["date_installe"])){
			$requete.=",date_installe";
		}
		$requete.=",createOn,updateOn) VALUES(:id_materiel,:id_modele,:sn";
		if (!empty($_POST["date_installe"])){			
			$requete.=",:date_installe";
		}
		$requete.=",now(),now())";		
		...
		$requete = "SELECT * FROM monitor WHERE id = :id";
		$prep=$db->prepare($requete);
	...
	{		
		...
		$requete = "UPDATE monitor SET id_modele=:id_modele,sn=:sn";
		if (!empty($_POST["date_installe"])){
			$tmpDate=explode('-',$_POST["date_installe"]);
			$_POST["date_installe"]="$tmpDate[2]-$tmpDate[1]-$tmpDate[0]";
		...
		$prep=$db->prepare($requete);
		$prep->bindParam(":id",$_POST["id_monitor"],PDO::PARAM_INT);
	...
	else if($_GET["action"] == "delete")
	{
		...
		$requete="DELETE FROM monitor WHERE id = :id;";		
		$prep=$db->prepare($requete);
		$prep->bindParam(":id",$_POST["id_monitor"],PDO::PARAM_INT);
		$prep->execute();
		$prep->closeCursor();
