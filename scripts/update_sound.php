...
try
{
	...
	{			
		...
		$requete="SELECT *,a.id as id_sound,b.id as id_marque,b.detail as marque,c.detail as modele,a.id_type as id_type_sound FROM sound a, marque b, modele c WHERE a.id_modele=c.id AND c.id_marque=b.id";// 
...
else if($_GET["action"] == "create")
	{
		...
		$prep = NULL;
		$requete = "INSERT INTO sound(id_materiel,id_modele,sn";
		...
		$requete = "SELECT * FROM sound WHERE id = :id";
		$prep=$db->prepare($requete);
	...
	{		
		...
		$requete = "UPDATE memory SET id_modele=:id_modele,sn=:sn";
		if (!empty($_POST["date_installe"])){
			$tmpDate=explode('-',$_POST["date_installe"]);
			$_POST["date_installe"]="$tmpDate[2]-$tmpDate[1]-$tmpDate[0]";
		...
		$prep=$db->prepare($requete);
		$prep->bindParam(":id",$_POST["id_sound"],PDO::PARAM_INT);
	...
	else if($_GET["action"] == "delete")
	{
		...
		$requete="DELETE FROM sound WHERE id = :id;";		
		$prep=$db->prepare($requete);
		$prep->bindParam(":id",$_POST["id_sound"],PDO::PARAM_INT);
		$prep->execute();
		$prep->closeCursor();
