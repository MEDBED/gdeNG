...
try
{
	...
	{			
		...
		$requete="SELECT *,a.id as id_storage,b.id as id_marque,b.detail as marque,c.detail as modele,a.id_type as id_type_storage FROM storage a, marque b, modele c WHERE a.id_modele=c.id AND c.id_marque=b.id";// 
		foreach ($tabFiltre as $search=>$champ){
			if (!empty($_POST[$champ])){$requete.=" AND $search LIKE :$champ";}
		}
		$requete.=" AND id_materiel=:id_materiel ORDER BY description LIMIT ".$_GET["jtStartIndex"]."," . $limHaute . ";";		
...
else if($_GET["action"] == "create")
	{
		...
		$prep = NULL;
		$requete = "INSERT INTO storage(id_materiel,id_modele,id_type,description,taille,sn,nom";
		if (!empty($_POST["date_installe"])){
			$requete.=",date_installe";
		}
		$requete.=",createOn,updateOn) VALUES(:id_materiel,:id_modele,:id_type,:description,:taille,:sn,:nom";
		if (!empty($_POST["date_installe"])){			
			$requete.=",:date_installe";
		}
		$requete.=",now(),now())";		
		...
		$prep->bindParam(":id_modele",$resMarque["id"],PDO::PARAM_INT);
                ...
                $prep->bindParam(":driver",$_POST["driver"],PDO::PARAM_STR);
                $prep->bindParam(":taille",$_POST["taille"],PDO::PARAM_STR);
		...
		$requete = "SELECT * FROM storage WHERE id = :id";
		$prep=$db->prepare($requete);
	...
	{		
		...
		$requete = "UPDATE storage SET id_modele=:id_modele,id_type=:id_type,description=:descritpion,taille=:taille,sn=:sn,nom=:nom";
		if (!empty($_POST["date_installe"])){
			$tmpDate=explode('-',$_POST["date_installe"]);
			$_POST["date_installe"]="$tmpDate[2]-$tmpDate[1]-$tmpDate[0]";
	...
	else if($_GET["action"] == "delete")
	{
		...
		$requete="DELETE FROM storage WHERE id = :id;";		
		$prep=$db->prepare($requete);
		$prep->bindParam(":id",$_POST["id_storage"],PDO::PARAM_INT);
		$prep->execute();
		$prep->closeCursor();
		$prep = NULL;		
		//Return result to jTable
