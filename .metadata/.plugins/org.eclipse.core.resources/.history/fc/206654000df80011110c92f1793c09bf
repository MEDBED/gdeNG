...
(function($) {
    ...
    $.fn.colorselect = function(option) {        
        var options = $.extend({
            hiddenid: null,
            title: "Couleur de fond de l'événement",
            width: 90,
            height: 90,           
            index: "-1",
        ...
        var blanka = '<a href="#" id="{1}-{0}" key="{2}" hidefocus="on"><em><span style=" text-align:center; width:46px;padding-top:2px; height:11px;border:solid 1px #8B7B8B;" unselectable="on">aucune</span></em></a>'
        var blank = [];
        blank.push("ffffff");
        blank.push(id);
        blank.push(-1);
